...
const inflightByServerUrl = new Map<string, number>();

type PersonaPlexFailureType =
  | "turn_timeout"
  | "busy"
  | "handshake_timeout"
  | "handshake_failed"
...
class PersonaPlexTurnError extends Error {
  public readonly code: PersonaPlexFailureType;
  constructor(code: PersonaPlexFailureType, message: string) {
    super(message);
...
export class PersonaPlexClient {
  ...
  async runTurn(args: PersonaPlexRunTurnArgs): Promise<PersonaPlexTurnResult> {
    ...
    const maybeTurnId = (args as any).turnId;
    const turnId =
      (typeof maybeTurnId === "string" && maybeTurnId.trim() ? maybeTurnId.trim() : undefined) ??
    ...
    const sessionKey = (() => {
      ...
      if (!roomId || !agentId) return undefined;
    ...
    const releaseCapacity = () => {
      ...
      if (n <= 1) inflightByServerUrl.delete(serverUrl);
      else inflightByServerUrl.set(serverUrl, n - 1);
    };
    ...
    const endOnce = (payload: { ok: boolean; failureType?: PersonaPlexFailureType; err?: Error; wsCloseCode?: number; wsCloseReason?: string }) => {
      if (ended) return;
      ended = true;
      releaseCapacity();
      ...
      ws.on("message", (data, isBinary) => {
        ...
        if (kind === 0x00) {
          ...
          if (handshakeAt === 0) {
            ...
            logger.info(
              { event: "PERSONAPLEX_HANDSHAKE_OK", turnId, handshakeMs: handshakeAt - (wsOpenedAt || startedAt) },
              "PersonaPlex handshake completed"
            );
          }
          return;
        }
        ...
        if (kind === 0x01) {
          ...
          if (firstAudioAt === 0) {
            ...
            logger.info(
              { event: "PERSONAPLEX_FIRST_AUDIO", turnId, firstAudioMs: firstAudioAt - startedAt },
              "PersonaPlex first audio received"
            );
          }
      ...
      ws.on("open", async () => {
        ...
        try {
          ...
          const handshakeTimeoutMs = Math.min(
            PERSONAPLEX_HANDSHAKE_TIMEOUT_CEIL_MS,
            Math.max(PERSONAPLEX_HANDSHAKE_TIMEOUT_FLOOR_MS, Math.floor(turnTimeoutMs * 0.8))
          );
