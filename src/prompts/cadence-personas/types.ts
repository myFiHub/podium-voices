...
export interface WpmTargets {
  ...
  connective: { min: number; max: number };
}

/** Pause durations in ms. */
export interface PauseTiers {
  /** Comma / clause boundary. */
  commaMs: { min: number; max: number };
  /** Sentence end (period). */
  periodMs: { min: number; max: number };
  /** Paragraph / topic shift. */
  paragraphMs: { min: number; max: number };
  /** Lead-in before an emphasized key clause. */
  emphasisLeadInMs: { min: number; max: number };
}
