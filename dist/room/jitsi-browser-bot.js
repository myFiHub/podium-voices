...
class JitsiBrowserBot {
    ...
    botDiag = process.env.BOT_DIAG === "1";
    ...
    lastNodeRoomAudioMaxAbs = 0;
    lastTtsFrameSentAt = 0;
    ...
    startBotDiagIfEnabled() {
        if (!this.botDiag)
            return;
        if (this.diagStartedAt > 0)
            return;
        ...
        logging_1.logger.info({ event: "BOT_DIAG_EXIT_NOTE", durationMs: this.botDiagDurationMs }, "BOT_DIAG: process will exit after duration with verdict (OK → exit 0, otherwise exit 2)");
        if (this.diagTimer)
    ...
    finishBotDiag() {
        ...
        if (!hasInbound)
            ...
            receiveVerdict = "INBOUND_RTP_BUT_PREMIX_SILENT";
        else if (!postmixOk)
            receiveVerdict = "PREMIX_OK_BUT_MIXER_SILENT";
        else if (!nodeOk)
            receiveVerdict = "MIXER_OK_BUT_NODE_SILENT";
    ...
    handleTruthProbe(msg) {
        const now = Date.now();
        const inboundBytesDelta = typeof msg.audio_inbound_bytes_delta === "number" ? msg.audio_inbound_bytes_delta : 0;
        const inboundPacketsDelta = typeof msg.audio_inbound_packets_delta === "number" ? msg.audio_inbound_packets_delta : 0;
        ...
        if (this.diagStatsStream) {
            try {
                this.diagStatsStream.write(JSON.stringify({
                    node_ts: now,
                    sessionId: this.sessionId,
                    conferenceId: this.conferenceId,
                    ...msg,
    ...
    async start() {
        ...
        this.wss.on("connection", (ws) => {
                ...
                try {
                    ...
                    if (msg.type === "join_result") {
                        ...
                        try {
                            ...
                            fs.writeFileSync(outPath, wav);
                            logging_1.logger.warn({ event: "AUDIO_WAV_SAVED", sessionId: this.sessionId, conferenceId: this.conferenceId, where: label, path: outPath, bytes: wav.length }, "Saved page WAV capture");
            ...
            logging_1.logger.info({ event: "BOT_JOIN_SENT", sessionId: this.sessionId, conferenceId: this.conferenceId, domain: host, xmppDomain: this.config.xmppDomain, roomName: this.config.roomName }, "Sent join to bot page");
        });
